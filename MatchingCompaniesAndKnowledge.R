...
#write.csv(CompanyMatches, 'CompanyMatches.csv', row.names = F)

# Column-wise matches for companies and knowledge
KnowlegdeMatches <- read.csv('KnowledgeMatches100.csv', header = T)
#CompanyMatches <- read.csv('CompanyMatches.csv', header = T)

# Look at all non-'' cells in company matches and see if they appear in 
# knowledge matches. If they do, keep. If they do not, change to ''.
